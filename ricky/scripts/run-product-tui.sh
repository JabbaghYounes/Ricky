...
export RICKY_TUI_START=$(date +%s)

# Create a log file for pipeline output
PIPELINE_LOG=$(mktemp "${TMPDIR:-/tmp}/ricky-pipeline.XXXXXX.log")

echo "Starting pipeline (log: $PIPELINE_LOG)..."

# Launch the pipeline in the background
"$RICK_DIR/scripts/run-product.sh" "$@" > "$PIPELINE_LOG" 2>&1 &
PIPELINE_PID=$!
...
cleanup() {
  if kill -0 "$PIPELINE_PID" 2>/dev/null; then
    ...
    echo "Re-attach TUI: $RICK_DIR/scripts/tui.sh 2 $PIPELINE_PID"
  else
    echo ""
...
"$RICK_DIR/scripts/tui.sh" 2 "$PIPELINE_PID"
